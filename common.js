...
async function loadCategory() {
  try {
    ...
    data.forEach((item) => {
      ...
      const link = `
                                    <a href="${pageLink}?slug=${item.slug}" 
...
async function loadAboutusToggle() {
  ...
  try {
    ...
    data.forEach((item) => {
      ...
      const link = `
        <a href="/whychoose.html?slug=${item.slug}" 
           class="block px-4 py-3 text-slate-700 hover:bg-[#d5e880]/20">
           ${item.pageName}
        </a>
      `;
...
document.addEventListener("submit", function (e) {
  if (e.target.id === "enquiryForm") {
    ...
    fetch("https://glacier-treks-adventures-admin-back.vercel.app/api/enquiries", {
      method: "POST",
      headers: {
